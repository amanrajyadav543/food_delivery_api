import { EditCustomerProfileInput, CartItem } from './../dto/Customer.dto';
...
export const CustomerSignUp=async(req: Request, res: Response, next:NextFunction)=>{
    ...
    return res.status(400).json({msg:"Error while create user"});
...
export const RequestOtp=async(req:Request, res:Response,next:NextFunction)=>{
    ...
    if(customer){
        ...
        if(profile){
            ...
            return res.status(200).json({message:'OTP sent to your registerd Mobile Number'});
...
export const GetCustomerProfile=async(req:Request, res:Response, next:NextFunction)=>{
    ...
    return res.status(400).json({msg:"Error while Fetching the Profile"});
}
export const EditCustomerProfile=async (req:Request, res:Response, next:NextFunction)=>{
    const customer= req.user;
    const customerInputs=plainToClass(EditCustomerProfileInput,req.body);
    const validateError = await validate(customerInputs,{validateError:{target:true}})
